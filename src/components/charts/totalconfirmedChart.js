...
function TotalConfirmedChart(props) {
    ...
    props.timeseries.forEach((data, index) => {
        if (index >= 31) {
            const date = parse(data.date, 'yyyy-MM-dd', new Date(2020, 0, 1));
            dates.push(date);
            confirmed.push(data.totalCases);
            recovered.push(data.totalRecoveries);
            deceased.push(data.totalDeaths);
        }
    });
